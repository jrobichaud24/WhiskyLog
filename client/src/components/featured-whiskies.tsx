...
export default function FeaturedWhiskies() {
  const { data: whiskies, isLoading, error } = useQuery<Whisky[]>({
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-12">
          {whiskies?.map((whisky, index) => (
            <Card 
              ...
              data-testid={`card-whisky-${whisky.id}`}
            >
              <CardContent className="p-6">
                <div className="w-full h-40 bg-amber-100 rounded-lg mb-4 flex items-center justify-center">
                    ...
                    <div className="flex text-amber-400">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star 
                          ...
                          data-testid={`star-${whisky.id}-${star}`}
                        />
                      ))}
                    </div>
                  </div>
                  <p className="font-playfair font-semibold text-gray-800" data-testid={`text-whisky-name-${whisky.id}`}>
                    {whisky.name}
                  </p>
                  <p className="text-sm text-gray-600" data-testid={`text-whisky-region-${whisky.id}`}>
                    {whisky.region}
                  </p>
                </div>
              </CardContent>
