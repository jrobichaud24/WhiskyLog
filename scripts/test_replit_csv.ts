...
// Copied from client/src/lib/csvUtils.ts to avoid import issues
...
console.log("Testing Replit CSV parsing...");
...
if (data.length > 0) {
    ...
    // Validation
    if (first.distillery && first.distillery.length > 30) {
        console.log("✅ Distillery ID preserved:", first.distillery);
    } else {
        console.error("❌ Distillery ID issue:", first.distillery);
    }

    if (first.abvPercent === "40.0") {
        console.log("✅ ABV parsed correctly as string");
    } else {
        console.error("❌ ABV issue:", first.abvPercent);
    }
}
